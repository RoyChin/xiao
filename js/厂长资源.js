...
let htmlurl = pdfh(htmlsearch, '.w-search-form&&action');

let hhtml=request(htmlurl + "?q=" + KEY + "&f=_all&p=" + MY_PAGE,{withHeaders:true,headers:{Cookie:cookie}});
...
lis.forEach(function(it){
    VODS.push({
        vod_id: pd(it,'a&&href',htmlurl),
        vod_name: pdfh(it,'h3.dytit&&Text'),
        vod_pic: pd(it,'img&&src',htmlurl),
